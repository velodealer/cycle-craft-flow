...
2. **Show live-update status on the InspectABike card** (`InspectABikeIntegration.tsx`).
  ...
  - Add a small line on the card: "Live updates: on" when a secret is held (per dealer or platform-wide), "Live updates: not receiving — ask InspectABike to send updates to &nbsp;" otherwise.
