...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: part, error } = await supabase.from('parts')
      .select('id, business_id, bike_id, description, brand, cost_price, fit_xero_posting_id, bikes:bike_id(reference)')
      .eq('id', partId).maybeSingle();
    if (error) throw new Error(error.message);
    if (!part || part.business_id !== businessId || !part.bike_id) return json({ error: 'Fitted part not found' }, 404);
    ...
    const lines = fitPartJournalLines(Number(part.cost_price || 0), auth.settings.accounts ?? {}, description);
    ...
    const { error: recErr } = await supabase.from('parts').update({
      fit_xero_posting_id: journalId, fit_xero_sync_status: 'synced', fit_xero_sync_error: null,
    }).eq('id', partId);
