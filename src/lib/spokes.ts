...
export async function upsertComponentsForBike(bikeId: string, components: MappedComponent[]): Promise<LinkResult> {
  ...
  if (!bikeId || total === 0) return { linked: 0, total, failed: [] };
  // One server-side, all-or-nothing save: closing the app can't cut it off halfway.
  const payload = components.map((c) => ({
    slot: c.slot, categorySlug: c.categorySlug, brand: c.brand, model: c.model,
    mpn: c.mpn || null, description: c.description || null, position: c.position || null,
    weightG: c.weightG ?? null, attributes: c.attributes || {},
  }));
  const { data, error } = await (supabase as any).rpc('link_bike_components', { _bike_id: bikeId, _components: payload });
  if (error) {
    console.error('[99spokes] parts not saved', bikeId, error);
    return { linked: 0, total, failed: [{ slot: 'all parts', reason: error.message }] };
  }
  return { linked: Number(data?.linked ?? 0), total, failed: [] };
}
